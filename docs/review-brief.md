...
- Wired Vercel builds through `vercel.json` so production deployments run `npm run db:migrate:production` before `next build`.
- Updated the README, launch runbook, env example, and handoff docs so operators know the exact migration step and Vercel build path before first production launch and after future schema changes.
...
- result: production migration command is now explicit, the deploy alias fails fast with a clear error when `DIRECT_DATABASE_URL` is missing, and Vercel has a checked-in build path that runs migrations before build
...
- Set `DIRECT_DATABASE_URL` on the Vercel project if it is missing, trigger a new deployment from `main`, and then retry Google OAuth login on the live site.
